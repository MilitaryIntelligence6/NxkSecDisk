...
public class FileServiceImpl
    ...
    public String doRenameFile(final HttpServletRequest request) {
        ...
        if (!TextFormatUtil.getInstance().matcherFileName(newFileName) || newFileName.indexOf(".") == 0) {
            return ERROR_PARAMETER;
        }
            ...
            for (final String id : idList) {
                ...
                if (nodeMapper.queryByParentFolderId(locationpath).parallelStream()
                        ...
                        case "cover":
                            ...
                            if (nodeMapper.deleteById(n.getFileId()) > 0) {
                                ...
                                if (isCopy) {
                                    ...
                                    Node copyNode = fileBlockUtil.insertNewNode(node.getFileName(), account,
                                            node.getFilePath(),
                                    ...
                                    // 成功，记录日志
                                    this.logUtil.writeMoveFileEvent(account, ip, originPath,
                                            fileBlockUtil.getNodePath(node), isCopy);
                                }
                    ...
                    if (isCopy) {
                        ...
                        // 成功后，记录日志
                        this.logUtil.writeMoveFileEvent(account, ip, originPath, fileBlockUtil.getNodePath(node),
                                isCopy);
                    }
                }
                    ...
                    switch (optMap.get(fid)) {
                        case "cover": {
                            ...
                            if (folderMapper.deleteById(f.getFolderId()) > 0) {
                                ...
                                if (isCopy) {
                                    ...
                                    Folder newFolder = folderUtil.copyFolderByNewNameToPath(folder, account,
                                            targetFolder, null);
                                    ...
                                    if (newFolder != null) {
                                        ...
                                        this.logUtil.writeMoveFolderEvent(account, ip, originPath,
                                                folderUtil.getFolderPath(newFolder),
                                                isCopy);
                                        break;
                                    }
                                } else {
                                    ...
                                    if (this.folderMapper.update(folder) > 0) {
                                        ...
                                        // 成功后，记录日志
                                        this.logUtil.writeMoveFolderEvent(account, ip, originPath,
                                                folderUtil.getFolderPath(folder),
                                                isCopy);
                                        break;
                                    }
                                }
                        ...
                        case "both": {
                            ...
                            if (isCopy) {
                                ...
                                // 成功，记录日志
                                this.logUtil.writeMoveFolderEvent(account, ip, originPath,
                                        folderUtil.getFolderPath(folder), isCopy);
                            }
                            // 保留两者成功，继续后面的操作
                            break;
                        }
