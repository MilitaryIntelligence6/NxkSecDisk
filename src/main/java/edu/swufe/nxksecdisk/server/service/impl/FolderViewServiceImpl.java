	...
	public String getFolderViewToJson(final String fid, final HttpSession session, final HttpServletRequest request) {
		...
		for (Folder f : folders) {
			if (config.accessFolder(f, account)) {
				if (f.getFolderHomework() == 1 && account == null) {
					String currentTime = ServerTimeUtil.accurateToSecond();
					SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
					Date current;
					try {
						current = format.parse(currentTime);
						Date start = format.parse(f.getFolderHomeworkStartTime());
						Date end = format.parse(f.getFolderHomeworkEndTime());
						if (current.before(start) || current.after(end)) {
							f.setIsUploadByTime(0);// 设置为0为不允许上传，用于在前端显示开锁和解锁图标
						}
