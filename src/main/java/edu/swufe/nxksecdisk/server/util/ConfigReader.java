...
public class ConfigReader {
	...
	private static final String ACCOUNT_PROPERTIES_FILE = "account.properties";
	...
	private static final String DEFAULT_LOG_LEVEL = "E";

	private static final String DEFAULT_VC_LEVEL = "STANDARD";

	private static final String DEFAULT_MUST_LOGIN = "O";

	private final String DEFAULT_FILE_SYSTEM_PATH;

	private static final String DEFAULT_FILE_SYSTEM_PATH_SETTING = "DEFAULT";

	private static final String DEFAULT_ACCOUNT_ID = "admin";

	private static final String DEFAULT_ACCOUNT_PWD = "000000";

	private static final String DEFAULT_ACCOUNT_AUTH = "cudrm";

	private static final String DEFAULT_AUTH_OVERALL = "l";

	private static final String DEFAULT_PASSWORD_CHANGE_SETTING = "N";

	private static final String DEFAULT_FILE_CHAIN_SETTING = "CLOSE";
	...
	private ConfigReader() {
		...
		try {
			...
			// 顺序问题, 先加载会出问题;
			// AppSystem.out.printf("错误：无法加载一个或多个配置文件（位于%s路径下），请尝试删除旧的配置文件并重新启动本应用或查看安装路径的权限（必须可读写）。",
			// this.confdir);
		}
	}
	...
	public boolean authorized(final String account, final AccountAuth auth, List<String> folders) {
		if (account != null && account.length() > 0) {
			...
			for (String id : folders) {
				...
				if (addedAuth != null) {
					auths.setLength(0);	// 登录用户仅获得特定文件夹下的权限，而不包括自己的权限、匿名用户的权限
			...
			final StringBuffer auths2 = new StringBuffer();
			final String overall2 = this.accountp.getProperty("authOverall");
			if (overall2 != null) {
				auths2.append(overall2);
			}
			for (String id : folders) {
				String addedAuth = accountp.getProperty(String.format("authOverall.auth.%s", id));
				if (addedAuth != null) {
					auths2.setLength(0); // 匿名用户仅获得特定文件夹下的权限，而不包括匿名权限
					auths2.append(addedAuth);
				}
			}

			switch (auth) {
			case CREATE_NEW_FOLDER: {
				return auths2.indexOf("c") >= 0;
			}
			case UPLOAD_FILES: {
				return auths2.indexOf("u") >= 0;
			}
			case DELETE_FILE_OR_FOLDER: {
				return auths2.indexOf("d") >= 0;
			}
			case RENAME_FILE_OR_FOLDER: {
				return auths2.indexOf("r") >= 0;
			}
			case DOWNLOAD_FILES: {
				return auths2.indexOf("l") >= 0;
			}
			case MOVE_FILES: {
				return auths2.indexOf("m") >= 0;
			}
			default: {
				return false;
			}
			...
			if ("true".equals(enableHttps)) {
				...
				if (keyFile.isFile()) {
					...
					if (keyFile.isFile()) {
						...
						AppSystem.out.println("错误：无法启用https支持，因为kiftd未能找到https证书文件。您必须在应用主目录内放置PKCS12（必须命名为https"
		...
		accountPropertiesUpdateDaemonRunnable = () -> {
			try {
				while (true) {
					...
					confPath.register(ws, StandardWatchEventKinds.ENTRY_MODIFY, StandardWatchEventKinds.ENTRY_DELETE,
