...
import kohgylw.kiftd.server.pojo.CheckImportFolderResponds;
import kohgylw.kiftd.server.pojo.CheckUploadFilesResponds;
...
public class FileServiceImpl extends RangeFileStreamWriter implements FileService {
	...
	public String checkUploadFile(final HttpServletRequest request, final HttpServletResponse response) {
		...
		// 权限检查
		if (!ConfigureReader.getInstance().authorized(account, AccountAuth.UPLOAD_FILES, fu.getAllFoldersId(folderId))
					...
					if (ConfigureReader.getInstance().accessFolder(folder, account)) {
						...
						if (fo != null) {
							int status = writeRangeFileStream(request, response, fo, f.getFileName(), CONTENT_TYPE,
									ConfigureReader.getInstance().getDownloadMaxRate(account), fbu.getETag(fo), true);
							// 日志记录（仅针对一次下载）
		...
		if (zipname != null && !zipname.equals("ERROR")) {
			...
			if (zip.exists()) {
				writeRangeFileStream(request, response, zip, fname, CONTENT_TYPE,
						ConfigureReader.getInstance().getDownloadMaxRate(account), fbu.getETag(zip), true);
				zip.delete();
			}
		}
	}
			...
			try {
				...
				for (final String fid : idList) {
					...
					if (ConfigureReader.getInstance().authorized(account, AccountAuth.DOWNLOAD_FILES,
							...
							&& ConfigureReader.getInstance().accessFolder(flm.queryById(n.getFileParentFolder()),
									account)) {
	...
	public String doMoveFiles(HttpServletRequest request) {
		...
		// 以及是否具备操作权限
		if (!ConfigureReader.getInstance().authorized(account, AccountAuth.MOVE_FILES, fu.getAllFoldersId(locationpath))) {
			return NO_AUTHORIZED;
		}
		...
		try {
			...
			for (final String id : idList) {
				...
				// 权限检查
				if (!ConfigureReader.getInstance().accessFolder(flm.queryById(node.getFileParentFolder()), account)) {
